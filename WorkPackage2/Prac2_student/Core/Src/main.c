...
static uint8_t i = 0;       // Used to index through the patterns array.
static uint8_t state = 0;   // Used to check the default state of the switch, which is 0 when not pressed.
...
int main(void)
{
  ...
  for (uint8_t x=address; x<6;x++ ){
	  write_to_address(x, patterns[x]);
  }
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t data_output = read_from_address(i);
	if (data_output == patterns[i]) {GPIOB->ODR = patterns[i];}
	else {GPIOB->ODR &= 0x01;}
	i++;
	if(i>5){ i=0;}
}
